...
def test_aiding_create():
    ...
    (when(eventing).incept(keys=keys, isith='1', nsith='1', ndigs=ndigs, code='E', wits=[], toad='0', data=[])
    ...
    when(mock_client).post('/identifiers', json={'name': 'new_aid', 'icp': {'a': 'key event dictionary'},
...
def test_aiding_create_delegation():
    ...
    from keri.core import eventing
    when(eventing).delcept(keys=['a signer verfer qb64'], delpre='my delegation', isith='1', nsith='1',
    ...
    resp = mock({'json': lambda: {'post': 'success'}})
    when(mock_client).post('/identifiers',
...
def test_aiding_interact_with_data():
    ...
    from keri.core import eventing, coring
    mock_serder = mock({'ked': {'a': 'key event dictionary'}, 'raw': b'serder raw bytes'}, spec=coring.Serder)
    when(eventing).interact('hab prefix', sn=1, data=[{'some': 'data'}, {'some': 'more'}], dig='hab digest').thenReturn(
        mock_serder)
...
def test_aiding_rotate():
    ...
    keys = ['key1']
    ndigs = ['ndig1']
    when(mock_keeper).rotate(ncodes=['A'], transferable=True, states=[{'i': 'state 1'}, {'i': 'state 2'}],
    ...
    from keri.core import eventing
    when(eventing).rotate(pre='hab prefix', keys=['key1'], dig='hab digest', sn=1, isith='1', nsith='1',
