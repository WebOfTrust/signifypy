...
def test_registries():
    ...
    mock_hab = {'prefix': 'a_prefix', 'state': {'s': '1', 'd': "ABCDEFG"}}
    name = "aid1"
    regName = "reg1"
    ...
    expect(mock_client, times=1).post(path=f"/identifiers/{name}/registries", json=ANY())

    from signify.app.credentialing import Registries

    Registries(client=mock_client).create(name=name, registryName=regName)
