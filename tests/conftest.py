...
from keri.app import habbing, delegating, forwarding, indirecting, agenting, keeping, oobiing, \
    grouping, notifying, signing
from keri.app.habbing import openHby, Habery, Hab
from keri.core import coring, eventing, serdering, Salter, scheming, parsing
...
class GrantContainer(doing.DoDoer):
    ...
    def recur(self, tock=0.0, **opts):
        if not self.grants:
            return True
        super(GrantContainer, self).recur()
        return False
